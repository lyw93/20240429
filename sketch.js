...
function draw() {
  ...
  //span=15
  push()
    translate(width/2-capture_width/2, height/2-capture_height/2)//把原點移動(width/2-160, height/2-120)
       ...
       for(var y = 0;y<captureGraphics.height;y=y+span){
        ...
        if(radioElement.value()=="亮度"){
         ...
         // ellipse(x,y,span) //span代表圓圈直徑
          ellipse(x,y,span*map(bk,0,255,0,1)) //透過map指令，bk在0~255間，依照比例設定0~1間數字
        }
